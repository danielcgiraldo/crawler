...
def get_og(url):
    try:
            ...
            # Set headers to avoid rejection from websites
            ...
            # If the response is 404, raise an exception
            if response.status_code == 404:
                raise Exception(404)

            # If the response is 403, try again with the same session (preserves cookies generated by the website)
            if response.status_code == 403:
                response = session.get(url, headers=headers)
            
            # If the response is still 403 or it is 404, raise an exception
